...
if(isset($action) and $action=='add')
{
	if(empty($txtName))
	{
		ShowMsg('昵称不能为空！','product.php?cid=21');
		exit();
	}
	
	if(empty($txtAreaVraag))
	{
		ShowMsg('内容不能为空！','product.php?cid=21');
		exit();
	}
	...
	{
		ShowMsg('留言成功，感谢您的支持！','product.php?cid=21');
		exit();
	}
}
?>
...
<DIV class=repeat-bg-1>
    <DIV style="Z-INDEX: 9999; POSITION: relative" class=main-wrapper>
        <DIV class="cm-fl quotes">
            ...
            <DIV class="cm-fl q-text"><span>您当前所在位置：<?php echo GetPosStr($cid); ?></span></DIV>
        </DIV>
            ...
            <?php
                ...
                <DIV class="cm-fl box-1">
                    <DIV class="cm-fl d-box1 img-1">
                        ...
                        <SPAN style="COLOR: #00b0f5" class=title>&nbsp;<?php echo ReStrLen($row['title'],5); ?></A></SPAN><BR>
                        <SPAN>
                            <div style="float: left;width:100%;height:70px;margin-top:5px;"><?php echo ReStrLen($row['abstract_cn'],70); ?> </div>
                        
                         <HR>
                         <?php echo ReStrLen($row['abstract_en'],70); ?> 
                        </SPAN>
                    </DIV>
                </DIV>
                <?php
