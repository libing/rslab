...
<form name="form" id="form" method="post" action="goodstype_save.php">
	...
	function Show($id=0, $i=0)
	{
		...
		while($row = $dosql->GetArray($id))
		{
			...
			//显示属性
			if(isset($row['attrstr']) && $row['attrstr']!='-1')
			{
				$classname .= '<span class="titflag" title="包含的商品属性">';
				$attrs = explode(',',$row['attrstr']);

				if(is_array($attrs))
				{
					foreach($attrs as $v)
					{
						$r = $dosql->GetOne("SELECT `classname` FROM `#@__goodsattr` WHERE `id`=$v");
