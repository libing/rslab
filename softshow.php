	...
	<div class="TwoOfTwo">
		...
		<div class="softConts">
			...
			<div class="preNext">
				...
				//获取上一篇信息
				$r = $dosql->GetOne("SELECT * FROM `#@__soft` WHERE classid=".$row['classid']." AND id<".$row['id']." AND delstate='' AND checkinfo=true ORDER BY orderid DESC");
				if($r < 1)
				{
				...
				//获取下一篇信息
				$r = $dosql->GetOne("SELECT * FROM `#@__soft` WHERE classid=".$row['classid']." AND id>".$row['id']." AND delstate='' AND checkinfo=true ORDER BY orderid ASC");
				if($r < 1)
				{
